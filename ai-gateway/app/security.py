...
THE SCHEME IS NOT NEGOTIABLE HERE. custom_ai_bridge already signs its calls
...
logger = logging.getLogger("ai_gateway.security")
...
class HMACMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request, call_next):
        ...
        # Which tenant asked. custom_ai_bridge sets this from env.cr.dbname.
        # It is used for metrics and logs ONLY -- this service reads no
        # database, so the header cannot widen what an answer may contain. What
        # keeps an answer inside one tenant is the schema_hint fence in
        # main.py, not this string.
        request.state.tenant = request.headers.get("X-Tenant-Id", "unknown")
